...
test_that("patchFilter() with cells removes small patches", {
  ...
  n_small <- sum(raster::getValues(fp_small) == 1, na.rm = TRUE)
  n_large <- sum(raster::getValues(fp_large) == 1, na.rm = TRUE)

  expect_gte(n_small, n_large)
  expect_s4_class(fp_small, "RasterLayer")
  expect_s4_class(fp_large, "RasterLayer")
})
...
test_that("patchFilter() with area removes patches below area threshold", {
  ...
  expect_equal(raster::getValues(fp_cells), raster::getValues(fp_area))
})
...
test_that("patchFilter() with directions=4 is more conservative than default (8)", {
  ...
  n8 <- sum(raster::getValues(fp8) == 1, na.rm = TRUE)
  n4 <- sum(raster::getValues(fp4) == 1, na.rm = TRUE)
...
test_that("patchFilter() output is a binary raster", {
  ...
  vals <- unique(raster::getValues(fp))
