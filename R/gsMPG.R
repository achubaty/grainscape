...
gsMPG <- function(cost, patch, sa = NULL, filterPatch = NULL, spreadFactor = 0) {
  ...
  extractGraphSCN <- readLines(paste0(selesPath, "/extractgraph.scn"), n = -1)
  extractGraphSEL <- readLines(paste0(selesPath, "/extractgraph.sel"), n = -1)
  ...
  if ((class(selesGraph) == "try-error") || (nrow(selesGraph) == 0)) {
    ...
    mpg$patchId[] <- paste0(outputFolder, "\\patchid.asc") %>%
      raster() %>%
      getValues()

    mpg$voronoi[] <- paste0(outputFolder, "\\voronoi.asc") %>%
      raster() %>%
      getValues()

    mpg$lcpLinkId[] <- paste0(outputFolder, "\\linkidmpg.asc") %>%
      raster() %>%
      getValues()

    mpg$lcpPerimWeight[] <- paste0(outputFolder, "\\linkweightmpg.asc") %>%
      raster() %>%
      getValues()

    mpg$lcpPerimType[] <- paste0(outputFolder, "\\linktype.asc") %>%
      raster() %>%
      getValues()
