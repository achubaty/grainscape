#' Extract a minimum planar graph (MPG) model in strips from a landscape resistance surface
#'
#' @description
#' This function is used to extract a minimum planar graph (MPG) in strips,
#' and can be used as an alternative to \code{\link{gsMPG}}.
#' It can save time where there are a large number of patches by dividing the
#' landscape into a number of strips and stitching them back together.
#' It can also extract MPGs for each strip in parallel (with the \code{\link{parallel} package}).
#' Using this function maybe necessary when there are a very large number of patches (usually in excess of 2000).
#'
#' Stripping and extracting separate MPGs  may sometimes result in small artefactual
#' differences in graph topology and Voronoi tessellations.
#' Researchers may wish to quantify the impact of stripping by using a small
#' subregion of their landscape, and comparing the results from \code{gsMPG(x)}
#' and \code{gsMPGstrip(x, numStrips = 2, ...)}.
#'
#' Note that \code{gsMPGstitch} cannot be used to generate MPGs for use in lattice-based
#' grains of connectivity modelling.
#' \code{\link{gsMPG}} must be used for this purpose.
#'
#' @note
...
#' If the raster projection is one where cell sizes are approximately constant
#' in area (e.g., UTM), or the raster covers a relatively small geographic extent
#' (e.g., < 1000 km in dimension) areal measurements will often be adequate.
#' Reprojection of rasters should be considered to minimize these effects in other
#' cases (see \code{\link[raster]{projectRaster}}).
#'
#' @param cost  A raster of class \code{RasterLayer} giving a landscape resistance
#'              surface, where the values of each raster cell are proportional
#'              to the resistance to movement, dispersal, or gene flow for an
#'              organism in the landscape feature they represent.
#'              Missing values \code{NA} are acceptable (but see below).
#'              Negative values are not.  To extract an MPG with Euclidean links
#'              (i.e., and not least-cost path links) set \code{cost[] <- 1}.
#'
...
#'             If \code{cpu > 1} a cluster of up to this many processors will be
#'             initiated using the \code{parallel} package.
#'             Using this feature is recommended if processors are available.
#'             Typically one fewer processor than the available number of virtual
#'             cores works well (e.g., 7 on an 8-core i7 chip).
#'
#' @param outputFolder  Optional.  If not supplied this function creates files
#'                      for use by \code{SELES} in a temporary folder placed in
#'                      the R working directory that is deleted following successful execution.
#'                      Another location may be specified instead.
#'                      If supplied the location is not deleted after the analysis completes.
#'                      This can be useful for debugging purposes.
#'
#' @param filterPatch  Optional.  Remove patches from the analysis that are smaller than a given number of cells.
#'
...
#'
#' @param  selesPath  Deprecated; do not use. Maintained for backwards compatibility.
#'
#' @details
#' Use this function to create a minimum planar graph (MPG) that can be further
#' analyzed using \code{\link{igraph}} routines.\cr\cr
...
#' Vertex attributes give attributes of patches including patch area, the area of patch edges,
#' the core area of each patch, and the coordinates of the patch centroid.
#' All areal measurements are given as raster cell counts.
#' Edge attributes give attributes of the graph links including link weights giving
#' accumulated resistance/least-cost path distance, Euclidean distance, and the
#' start and end coordinates of each link.
#'
#' @references
...
#' frag <- raster(system.file("extdata/fragmented.asc", package = "grainscape2"))
...
#' fragMPGstitch <- gsMPGstitch(cost = fragCost, patchid = fragPatchId, numStrips = 3,
#'                              percentOverlap = 50, outputFolder = NULL, cpu = 2)
#' }
#'
gsMPGstitch <- function(cost, patchid, numStrips, percentOverlap,
                        disttype = "Cost", cpu = 1, outputFolder = NULL,
                        filterPatch = NULL, spreadFactor = 0, selesPath = NULL) {
  stop("gsMPGstitch() is currently in revision.  It does not work at this time.\n",
       "Please contact the authors for more information. ", call. = FALSE)
  rasCost <- cost
  rasPatchid <- patchid
  ...
  if ((class(cost) != "RasterLayer")) {
    stop("grainscape2: cost raster must be of class RasterLayer",
         call. = FALSE)
  }
  if ((class(patchid) != "RasterLayer")) {
    stop("grainscape2: patchid must be a raster (patch-based model)",
         call. = FALSE)
  }
  if (res(cost)[1] != res(cost)[2]) {
    warning(paste("grainscape2:  raster cells are not square;  assuming a square cell of ",
                  res(cost)[1], " units", sep = ""), call. = FALSE)
  }
  if (!is.na(projection(cost)) && (!grepl("UTM|utm", toupper(projection(cost))))) {
    warning("grainscape2:  projection suggests that all cells may not be of equal area; ",
  ...
  if (numStrips >= ncol(patchid)) {
    stop("grainscape2: number of strips must be less than the number of columns in the patchid raster", call. = FALSE)
  }
  if (percentOverlap > 100 | percentOverlap < 1) {
    stop("grainscape2: percentOverlap must be greater than 1 and less than 100", call. = FALSE)
  }
  ...
  for (i in 1:numStrips) {
    if (i < numStrips) {
      ...
      cropped_extent <- extent(x_starting, (x_starting + strip_width),
                               ymin(rasPatchid), ymax(rasPatchid))
      # create names for cropped maps
      strip_originalids <- paste0(outputFolder, "/", "PatchID", i, ".asc")
      cost_name <- paste0(outputFolder, "/", "Cost", i, ".asc")
  ...
  .doMPG <- function(whichMPG, outputFolder) {
    require(grainscape2)
    strip_patch <- raster(paste0(outputFolder, "/", "PatchID", whichMPG, ".asc"))
    strip_cost <- raster(paste0(outputFolder, "/", "Cost", whichMPG, ".asc"))
    MPG_name <- paste0("MPG", whichMPG)
    check <- class(try(gsMPG(cost = strip_cost, patch = strip_patch >= 1,
                             outputFolder = paste0(outputFolder, "/", MPG_name))))
  }

  if (cpu > 1) {
    useCpu <- min(cpu, numStrips)
    cat("Extracting MPG in each of", numStrips, "strips in parallel using", useCpu, "processors\n")
    cl <- parallel::makeCluster(useCpu)
    check <- parallel::parLapplyLB(cl, 1:numStrips, function(x) .doMPG(x, outputFolder))
  ...
  if (any(allCheck != "gsMPG")) {
    stop(paste("grainscape2:  Failure in MPG extraction of strip(s) ",
               paste(which(allCheck != "gsMPG"), sep = ", "), sep = ""), call. = FALSE)
  }
  ...
  linkId_start <- 1
  for (i in 1:numStrips) {
    strip_originalids <- raster(paste0(outputFolder, "/PatchID", i, ".asc"))

    # load in the patch id layer for this strip produced by MPG analysis
    MPG_name <- paste0("MPG", i)
    strip_newids <- raster(paste0(outputFolder, "/", MPG_name, "/patchid.asc"))
    ...
    patchid <- raster(paste0(outputFolder,"/", MPG_name, "/patchid.asc"))
    linkid <- raster(paste0(outputFolder,"/", MPG_name, "/linkidmpg.asc"))
    voronoi <- raster(paste0(outputFolder,"/", MPG_name, "/voronoi.asc"))
    ...
    writeRaster(raster(mpg_rasters_originalid, 1),
                paste0(outputFolder, "/", MPG_name, "/patchid_originalids.asc"), overwrite = TRUE)
    writeRaster(raster(mpg_rasters_originalid, 2),
                paste0(outputFolder, "/", MPG_name, "/voronoi_originalids.asc"), overwrite = TRUE)

    # Read in output text files from SELES mpg analysis
    patchStats <- read.table(paste0(outputFolder,"/", MPG_name, "/PatchStatsMPG.txt"), header = TRUE)
    linkStats <- read.table(paste0(outputFolder,"/", MPG_name, "/LinkStatsMPG.txt"), header = TRUE)
    ...
    # Replace new patch ids (note that here the patch ids are the same as the
    #   vector indices due to ordering) with original patch ids
    ...
    patchStats <- patchStats[, c("patchId_original", "nnSLDist", "nnDist", "nnCost",
                                 "Centroid", "size", "edge", "patchId")]
    patchStats <- patchStats[order(patchStats$patchId_original),]
    write.table(patchStats, paste0(outputFolder, "/", MPG_name, "/PatchStatsMPG_originalids.txt"), row.names = FALSE)
    ...
    for (j in 1:nrow(lookup_patchids)) {
      linkStats$nodeId1_original <- replace(linkStats$nodeId1_original,
                                            which(linkStats$nodeId1 == lookup_patchids[j, 1]), lookup_patchids[j, 2])
      linkStats$nodeId2_original <- replace(linkStats$nodeId2_original,
                                            which(linkStats$nodeId2 == lookup_patchids[j, 1]), lookup_patchids[j, 2])
    }
    ...
    ## C) Renumber link ids so that link ids are not the same in different strips
    ##    (SELES MPG analyses always numbers links from 1 to n)
    ...
    linkStats <- linkStats[, c("linkId_renumber", "linkId", "linkType",
                               "MINnodeId_original", "MAXnodeId_original",
                               "SLDist", "Dist", "Cost", "NumEdges", "NumSegments",
                               "StartLoc", "EndLoc", "nodeId1", "nodeId2",
                               "nodeId1_original", "nodeId2_original")]
    write.table(linkStats, paste0(outputFolder, "/", MPG_name, "/LinkStatsMPG_originalids.txt"), row.names = FALSE)
    ...
    writeRaster(linkid, paste0(outputFolder, "/", MPG_name, "/linkid_renumberids.asc"), overwrite = TRUE)

    rm(linkStats, patchStats, MPG_name, linkid, patchid, voronoi, strip_newids,
       strip_originalids, mpg_rasters_newid, mpg_rasters_originalid, j,
       lookup_patchids, lookup_linkIdrenumber)
  }
  ...
  for (i in 1:(numStrips - 1)) {
    ...
    # Strip 1
    MPG_name <- paste0("MPG", strip1)
    voronoi1 <- raster(paste0(outputFolder, "/", MPG_name, "/voronoi_originalids.asc"))
    if (i == 1) {
      linkid1 <- raster(paste0(outputFolder, "/", MPG_name, "/linkid_renumberids.asc"))
      linkStats1 <- read.table(paste0(outputFolder, "/", MPG_name, "/LinkStatsMPG_originalids.txt"), header = TRUE)
    }
    patchStats1 <- read.table(paste0(outputFolder, "/", MPG_name, "/PatchStatsMPG_originalids.txt"), header = TRUE)
    # Strip 2
    MPG_name <- paste0("MPG", strip2)
    voronoi2 <- raster(paste0(outputFolder, "/", MPG_name, "/voronoi_originalids.asc"))
    linkid2 <- raster(paste0(outputFolder, "/", MPG_name, "/linkid_renumberids.asc"))
    linkStats2 <- read.table(paste0(outputFolder, "/", MPG_name, "/LinkStatsMPG_originalids.txt"), header = TRUE)
    patchStats2 <- read.table(paste0(outputFolder, "/", MPG_name, "/PatchStatsMPG_originalids.txt"), header = TRUE)
    ...
    } else if (length(linkid1_loops) == 0) {
    ...
    } else if (length(linkid2_loops) == 0) {
        linkid2_deleted <- linkid2
    }

    # Delete any duplicate links (ie that connect the same two patches) from both
    # linkStats and linkid which were caused by cutting the study area into strips
    ...
    linkid1_duplicates <- subset(linkStats1_deleted$linkId_renumber,
                                 duplicated(linkStats1_deleted[, c("MINnodeId_original", "MAXnodeId_original")]))
    ...
    linkid2_duplicates <- subset(linkStats2_deleted$linkId_renumber,
                                 duplicated(linkStats2_deleted[,c("MINnodeId_original", "MAXnodeId_original")]))
    ...
    # Strip 1
    crop_extent1 <- extent(xmin(voronoi1) + x_shift, xmax(voronoi1),
                           ymin(voronoi1), ymax(voronoi1))
    voronoi1_overlap <- crop(voronoi1,crop_extent1)
    # Strip 2
    crop_extent2 <- extent(xmin(voronoi2), xmin(voronoi2) + x_shift,
                           ymin(voronoi2), ymax(voronoi2))
    ...
    # If the two voronoi maps are identical then the difference map will be zero throughout
    # and no links need to be deleted from either linkStats files or linkdid rasters
    if (cellStats(voronoi_difference, max) == 0) {
    ...
    } else if (cellStats(voronoi_difference, max) > 0) {
      ## Otherwise, if the two voronoi maps are not identical,
      ##   identify the voronois that differ and the links to be deleted.
      ...
      crop_splitoverlap <- extent(xmin(voronoi1_overlap) + x_shift, xmax(voronoi1_overlap),
                                  ymin(voronoi1_overlap), ymax(voronoi1_overlap))
      ...
      crop_splitoverlap <- extent(xmin(voronoi1_overlap), xmax(voronoi1_overlap) - x_shift,
                                  ymin(voronoi1_overlap), ymax(voronoi1_overlap))
      ...
      # Mask voronoi1_overlap_RHS with voronoi_difference_RHS to keep only those
      #   voronoi polygons from strip 1 on the RHS that are different.
      ...
      # Identify patch ids in strip 1 and strip 2 that differ on the RHS
      patchid1_deletions <- unique(c(unique(voronoi1_difference_RHS[]),
                                     unique(voronoi2_difference_RHS[])))

      # Mask voronoi1_overlap_RHS with voronoi_difference_RHS to keep only
      #   those voronoi polygons from strip 1 on the LHS that are different.
      ...
      # Identify patch ids in strip 1 and strip 2 that differ on the LHS
      patchid2_deletions <- unique(c(unique(voronoi1_difference_LHS[]),
                                     unique(voronoi2_difference_LHS[])))
      ...
      linkid1_deletions <- linkStats1_deleted[((linkStats1_deleted$MINnodeId_original %in% patchid1_deletions) == TRUE |
                                                 (linkStats1_deleted$MAXnodeId_original %in% patchid1_deletions) == TRUE),
                                              c("linkId_renumber", "MINnodeId_original", "MAXnodeId_original")]
      # 2) both end nodes are in strip 2
      linkid1_deletions <- linkid1_deletions[((linkid1_deletions$MAXnodeId_original %in% patchStats2$patchId_original) &
                                                (linkid1_deletions$MINnodeId_original %in% patchStats2$patchId_original)),]
      # 3) both end nodes are not in patchid2_deletions
      linkid1_deletions <- linkid1_deletions[((linkid1_deletions$MINnodeId_original %in% patchid2_deletions) == FALSE |
                                                (linkid1_deletions$MAXnodeId_original %in% patchid2_deletions) == FALSE),
                                             c("linkId_renumber", "MINnodeId_original", "MAXnodeId_original")]

      linkid1_deletions <- subset(linkid1_deletions,
                                  !duplicated(linkid1_deletions[, c("MINnodeId_original", "MAXnodeId_original")]))

      # Delete links from linkStats1 identified above
      linkStats1_deleted <- subset(linkStats1_deleted,
                                   !(linkStats1_deleted$linkId_renumber %in% linkid1_deletions$linkId_renumber))
      ...
      linkid2_deletions <- linkStats2_deleted[((linkStats2_deleted$MINnodeId_original %in% patchid2_deletions) == TRUE |
                                                 (linkStats2_deleted$MAXnodeId_original %in% patchid2_deletions) == TRUE),
                                              c("linkId_renumber","MINnodeId_original","MAXnodeId_original")]
      # 2) both end nodes are in strip 1
      linkid2_deletions <- linkid2_deletions[((linkid2_deletions$MAXnodeId_original %in% patchStats1$patchId_original) &
                                                (linkid2_deletions$MINnodeId_original %in% patchStats1$patchId_original)),]
      # 3) both end nodes are not in patchid1_deletions
      linkid2_deletions <- linkid2_deletions[((linkid2_deletions$MINnodeId_original %in% patchid1_deletions) == FALSE |
                                                (linkid2_deletions$MAXnodeId_original %in% patchid1_deletions) == FALSE),
                                             c("linkId_renumber", "MINnodeId_original", "MAXnodeId_original")]

      linkid2_deletions <- subset(linkid2_deletions,
                                  !duplicated(linkid2_deletions[, c("MINnodeId_original", "MAXnodeId_original")]))
      # delete duplicate links between strip 1 and strip 2
      alldeletions <- rbind(linkid1_deletions, linkid2_deletions)
      duplicate_deletions <- subset(alldeletions,
                                    duplicated(alldeletions[, c("MINnodeId_original", "MAXnodeId_original")]))
      ...
      linkStats2_deleted <- subset(linkStats2_deleted,
                                   !(linkStats2_deleted$linkId_renumber %in% linkid2_deletions$linkId_renumber))
  ...
  linkStats_merged <- linkStats_merged[, c("linkId_final", "linkType",
                                           "MINnodeId_original", "MAXnodeId_original",
                                           "SLDist", "Dist", "Cost",
                                           "NumEdges", "NumSegments",
                                           "StartLoc", "EndLoc",
                                           "nodeId1", "nodeId2",
                                           "nodeId1_original", "nodeId2_original",
                                           "linkId", "linkId_renumber")]
  write.table(linkStats_merged, file = paste0(outputFolder, "/LinkStatsMPG_merged.txt"), row.names = FALSE)

  writeRaster(voronoi_merged, filename = paste0(outputFolder, "/voronoi_merged.asc"), type = "ascii")
  ...
  writeRaster(linkid_merged, filename = paste0(outputFolder, "/linkidmpg_merged.asc"), type = "ascii")
  ...
  writeRaster(linklength_merged, filename = paste0(outputFolder, "/linklengthmpg_merged.asc"), type = "ascii")
  ...
  writeRaster(linkcost_merged, filename = paste0(outputFolder, "/linkcostmpg_merged.asc"), type = "ascii")
  ...
  rasX[] <- cellXY[, 1]
  rasY[] <- cellXY[, 2]
  centroids <- cbind(zonal(rasX, mpgstitch$patchId, fun = "mean"),
                     zonal(rasY, mpgstitch$patchId, fun = "mean")[, 2])
  ...
  toGraphE <- data.frame(v1 = linkStats_merged[, "MINnodeId_original"],
                         ...
                         linkId = linkStats_merged[, "linkId_final"],
                         lcpPerimWeight = linkStats_merged[, "Cost"],
                         eucPerimWeight = linkStats_merged[, "SLDist"],
                         lcpPerimLength = linkStats_merged[, "Dist"],
