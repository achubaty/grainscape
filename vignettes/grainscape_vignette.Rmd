...
author: 
- Paul Galpern^[Faculty of Environmental Design, University of Calgary, AB, Canada]
- Alex M. Chubaty^[Canadian Forest Service, Natural Resources Canada, Victoria, BC, Canada]
output:
  pdf_document:
    toc: no
...
vignette: >
  ...
  %\VignetteDepends{cowplot, igraph, raster, ggplot2}
...
This reference begins by introducing these two model types, and concludes by demonstrating a variety of `grainscape` models, visualizations, and analyses using a consistent visual style. Code and commentary are included throughout.
...
The resulting table is shown in \autoref{fig:components}.
...
ggplot() +
  ...
  geom_segment(data  = ggGS(patchyMPG, "links"),
               aes(x = x1, y = y1, xend = x2, yend = y2,  colour = lcpPerimWeight >= 250)) +
  scale_colour_manual(values = c("forestgreen", NA)) +
  geom_point(data = ggGS(patchyMPG, "nodes"), aes(x = x, y = y), colour = "darkgreen")
```
...
![A planar network with one-dimensional point nodes extracted using `grainscape`.
Nodes are represented as points and labelled with their `patchId`.
Links are line segments among nodes.](images/figure9.png)
...
figure9 <- plot(mpg, quick = "network", theme = FALSE) + 
...
![A planar network with one-dimensional nodes extracted on a resistance surface.
Nodes are represented by grey-coloured cells and labelled with their `patchId`.
Links are green paths between nodes, shown in a spatially-explicit representation.](images/figure10.png)
...
figure10 <- plot(mpg, quick = "mpgPlot", theme = FALSE) + 
  geom_text(data = ggGS(mpg, "nodes"),  aes(x = x + 3, y = y + 3, label = patchId)) +
...
The value of using patch perimeters rather than centroids is that it potentially improves the estimation fo the shortest paths among patches.
...
![A minimum planar graph (MPG) of a simulated resistance surface.
Focal patches are grey regions and green lines indicate spatially-explicit links among patches.
Several patches are labelled with their numerical `patchId`.](images/figure11.png)
...
![A minimum planar graph (MPG) with links represented from the perimeter of the patches (*i.e.*, at the start and end points of spatially-explicit links).
This rendering can simplify visualization](images/figure13.png)
...
![A minimum planar graph with centroid node and link representation where the width of the links has been scaled in proportion to the reduction in connectivity due to resistance.
Wider links correspond to reduced connectivity.](images/figure16.png)
...
![A link threshold representation of a minimum planar graph (MPG), where links connect the centroids of adjacent patches in the same component (cluster).
Nodes that form their own component are shown as circles without links](images/figure17.png)
...
Here, `grainscape::threshold()` is used to remove links longer than a certain length from the `igraph` model of the spatial network.
...
![A link threshold representation of a minimum planar graph (MPG), where links connect the centroids of adjacent patches in the same component (cluster).
Nodes that are part of a component are labelled with their component membership](images/figure18.png)
...
![A link threshold representation of a minimum planar graph (MPG), where nodes, plotted at patch centroids, are scaled in proportion to the degree of the node (*i.e.*, the number of links adjacent on the node).
Larger circles indicate nodes with a higher degree in the MPG.](images/figure19.png)
...
figure19 <- ggplot() +
  geom_raster(data = ggGS(fragMPG, "patchId"), aes(x = x, y = y, fill = value > 0)) +
  scale_fill_manual(values = "grey") +
  geom_segment(data  = ggGS(fragMPG, "links"),
               aes(x = x1, y = y1, xend = x2, yend = y2, colour = lcpPerimWeight > 20)) +
  scale_colour_manual(values = c("forestgreen", NA)) +
...
Finding the shortest path through the network from a source to a desination node and the length of that path is a useful prediction of the network model and has many applications [@Zeller:2012le; @Galpern:2012me; @Adriaensen2003] .
...
The Voronoi polygons that are complementary to a given scale or threshold of a spatial network describe a geographic area.
...
The black nodes and links demonstrate the corridor between the polygons containing the start and end points (plotted in red as `X`).
Green nodes and links show the remainder of the grains of connectivity network](images/figure24.png)
...
![A scaled patch grains of connectivity (GOC) model showing the outline of Voronoi polygons in grey, and the network connections among polygons as links plotted from adjacent polygon centroids.
The locations of eight focal points used to calculate a pairwise distance matrix are plotted in red](images/figure25.png)
...
ptsD2 <- round(ptsD2, 1)
dimnames(ptsD2)[[1]] <- paste0("Point ", 1:8, " (Polygon ", dimnames(ptsD2)[[2]], ")")
...
```{r, include=FALSE}
## save all vignette figures
doFig <- ls()[grepl("figure", ls())]
done <- lapply(doFig, function(fig) {
  ggsave(paste0("images/", fig, ".png"), plot = get(fig), width = 3, height = 3)
})
