...
The Minimum Planar Graph (MPG) algorithm is used to find the Voronoi boundaries and approximate least cost paths between patches.
...
Finding the least cost path this way is only possible with the use of storing the child cells, which will eventually become parent cells, in a queuing table that sorts the cells in a certain order.
The child cells are sorted by increasing Euclidean distance between the child cell and their origin cell, the perimeter cell that the connection originally spawned from.
