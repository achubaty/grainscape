---
title: "The `grainscape` package for R"
subtitle: "Tutorials using simulated landscapes"
...
plot(grain(patchyGOC, whichThresh = 62))
```
...
plot(grain(patchyGOCLattice, whichThresh = 35)@voronoi,
     col = sample(terrain.colors(100)))
```
...
For final analyses where visualization is a goal, build the GOC model with the `sp=TRUE` parameter set. **[TO DO: `sp` argument has been deprecated.]**
...
```{r, results='hide'}
#patchyGOC_SP <- GOC(patchyMPG, nThresh = 100) ## TO DO
#patchyGOCLattice_SP <- GOC(patchyMPGLattice, nThresh = 100) ## TO DO
```

Note how we can now visualize the Voronoi tessellation also with the `sp=TRUE` parameter to make things sharper. **[TO DO: `sp` argument has been deprecated.]**
...
#plot(grain(patchyGOCLattice_SP, whichThresh = 1, sp = TRUE)@voronoiSP,
#     add = TRUE) ## TO DO
...
#corridor1 <- corridor(patchyGOC_SP, whichThresh = 1, coords = loc, doPlot = TRUE) ## TO DO
...
#loc <- rbind(c(50, -350), c(350, -50))
#corridor1 <- corridor(patchyGOCLattice_SP, whichThresh = 45, coords = loc, doPlot = TRUE)
#points(loc, pch = c("P", "Q"), cex = 1.25)
#mtext(paste("Shortest path between P and Q:", round(corridor1$corridorLength, 2), "resistance units",
#            sep = " "),
#      side = 1)
```
...
#plot(tinyPatchCorridor$voronoiSP, col = "lightgrey", border = "white", lwd = 2) ## TO DO
